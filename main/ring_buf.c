...
static inline uint32_t nxt(uint32_t i, uint32_t cap){ return (i+1U) % cap; }

esp_err_t ring_buf_create(ring_buf_t **out, uint32_t cap, uint32_t slot_size, bool try_psram)
{
  if (!out || !cap || !slot_size) return ESP_ERR_INVALID_ARG;
  ring_buf_t *rb = heap_caps_malloc(sizeof(*rb), MALLOC_CAP_8BIT);
  if (!rb) return ESP_ERR_NO_MEM;
  memset(rb, 0, sizeof(*rb));
  rb->cap = cap; rb->slot_size = slot_size;

  rb->lock = xSemaphoreCreateMutex();
  if (!rb->lock) { free(rb); return ESP_ERR_NO_MEM; }
  ...
  if (!rb->slots) { vSemaphoreDelete(rb->lock); free(rb); return ESP_ERR_NO_MEM; }

  *out = rb; return ESP_OK;
}
